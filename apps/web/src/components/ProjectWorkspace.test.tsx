...
describe('ProjectWorkspace', () => {
  ...
  beforeEach(() => {
    ...
    onRenameProject.mockResolvedValue(undefined);
  ...
  it('renames the project from its workspace heading', async () => {
    renderWorkspace();
    fireEvent.click(screen.getByRole('button', { name: 'Rename Product launch' }));
    fireEvent.change(screen.getByRole('textbox', { name: 'Project name' }), { target: { value: 'Launch command' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save project name' }));
    await waitFor(() => expect(onRenameProject).toHaveBeenCalledWith(project, 'Launch command'));
  });
