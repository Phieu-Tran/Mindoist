...
import { ArrowRight, AtSign, CalendarClock, Check, Command as CommandIcon, CornerDownLeft, Flag, FolderOpen, ListChecks, Plus, Search, Timer } from 'lucide-react';
          ...
          <div className="command-quick-fields" aria-label={t('quickAdd.command.planTask')} data-testid="command-quick-fields">
            <div className="command-quick-fields-inner" data-testid={preview ? 'command-preview' : undefined}>
              ...
              <label className="command-quick-control is-project">
                ...
                <select aria-label={t('quickAdd.command.project')} data-testid="quick-add-project" name="quick-add-project" autoComplete="off" value={resolvedEdits.projectId} onChange={event => setEdits(current => ({ ...current, projectId: event.target.value }))}>
              ...
              <div className="command-date-shortcuts" aria-label={t('quickAdd.command.date')}>
                <button type="button" aria-pressed={resolvedEdits.date === today} className={resolvedEdits.date === today ? 'is-active' : ''} onClick={() => setEdits(current => ({ ...current, date: today }))}>{t('quickAdd.command.today')}</button>
                <button type="button" aria-pressed={resolvedEdits.date === tomorrow} className={resolvedEdits.date === tomorrow ? 'is-active' : ''} onClick={() => setEdits(current => ({ ...current, date: tomorrow }))}>{t('quickAdd.command.tomorrow')}</button>
              </div>
              <DatePicker
                value={resolvedEdits.date}
                onChange={date => setEdits(current => ({ ...current, date }))}
                placeholder={t('quickAdd.command.date')}
                testId="quick-add-date"
                className="command-picker command-date-picker"
                ariaLabel={t('quickAdd.command.date')}
                locale={locale ?? i18n.resolvedLanguage}
                todayLabel={t('quickAdd.command.today')}
                clearLabel={t('detail.clear')}
                previousMonthLabel={t('summary.previousMonth')}
                nextMonthLabel={t('summary.nextMonth')}
                onOpenChange={pickerOpen => { datePickerOpenRef.current = pickerOpen; }}
                portalContainerRef={commandRootRef}
              />
              <TimePicker
                value={resolvedEdits.time}
                onChange={time => setEdits(current => ({ ...current, time, ...(!resolvedEdits.date && time ? { date: today } : {}) }))}
                placeholder={t('quickAdd.command.time')}
                testId="quick-add-time"
                className="command-picker command-time-picker"
                ariaLabel={t('quickAdd.command.time')}
                pickerLabel={t('quickAdd.command.time')}
                allDayLabel={t('calendar.allDay')}
                onOpenChange={pickerOpen => { timePickerOpenRef.current = pickerOpen; }}
                portalContainerRef={commandRootRef}
              />
              <label className="command-quick-control is-priority">
                <Flag className="size-3.5" aria-hidden="true" />
                <select aria-label={t('quickAdd.command.priority')} data-testid="quick-add-priority" name="quick-add-priority" autoComplete="off" value={resolvedEdits.priority} onChange={event => setEdits(current => ({ ...current, priority: event.target.value }))}>
              ...
              {resolvedEdits.estimateMin && (
                <label className="command-quick-control">
                  ...
                  <input aria-label={t('detail.duration')} name="quick-add-estimate" autoComplete="off" inputMode="numeric" type="number" min="1" value={resolvedEdits.estimateMin} onChange={event => setEdits(current => ({ ...current, estimateMin: event.target.value }))} />
