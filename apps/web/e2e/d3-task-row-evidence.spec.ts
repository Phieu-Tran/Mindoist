...
async function arrangeScene(page: Page) {
  ...
  await page.getByTestId('add-task-input').fill('Plain task, no metadata');
  ...
  await page.getByTestId('add-task-input').fill(richTitle);
  await page.getByTestId('add-task-btn').click();
  await page.getByText(richTitle, { exact: true }).click();
