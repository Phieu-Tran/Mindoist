...
class MindoistDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(6)
      .stores({
        // Deadline and estimate are canonical and intentionally not indexed here.
        tasks: 'id, projectId, sectionId, parentId, completedAt, deletedAt, updatedAt',
      })
      .upgrade(async tx => {
        await tx.table('tasks').toCollection().modify(task => {
          if (task.estimateMin == null) task.estimateMin = task.durationMin ?? null;
          delete task.dueDate;
          delete task.dueTime;
          delete task.durationMin;
        });
      });
  }
}
