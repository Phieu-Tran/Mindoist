...
test('[B1.3] calendar priority colors, legend and tooltip remain responsive', async ({ page }) => {
  ...
  const urgentEvent = page.locator('.fc-event').filter({ hasText: 'A urgent launch' }).first();
  await expect(urgentEvent).toBeVisible();
  await expect(urgentEvent).toHaveAttribute('aria-label', /A urgent launch · P1/);
  await urgentEvent.hover({ force: true });
  await expect(urgentEvent.locator('[role="tooltip"]')).toBeVisible();
