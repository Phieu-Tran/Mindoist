...
import { User as UserIcon, Palette, LayoutList, Cloud, Upload, Info, Timer, Bell, Send } from 'lucide-react';
...
type SettingsCategory = 'account' | 'appearance' | 'navigation' | 'integrations' | 'google' | 'data' | 'productivity' | 'about';
...
interface Props {
  ...
  onUpdateHiddenNavItems: (hiddenNavItems: SidebarView[]) => void;
...
export function SettingsPage({
  ...
  hiddenNavItems, onUpdateHiddenNavItems,
  ...
  const categories: { id: SettingsCategory; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
    ...
    { id: 'navigation', label: t('settingsPage.navigation'), icon: LayoutList },
  ...
  return (
    <div className="flex max-w-[44rem] flex-col gap-6 sm:flex-row">
      ...
      <div className="min-w-0 flex-1">
        ...
        {category === 'navigation' && (
          <div data-testid="settings-panel-navigation">
            <SettingsView hiddenNavItems={hiddenNavItems} onUpdate={onUpdateHiddenNavItems} />
          </div>
        )}
