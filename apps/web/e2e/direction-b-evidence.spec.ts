...
async function addTask(page: Page, input: string, title: string) {
  ...
  await page.getByTestId('global-quick-capture-input').fill(input);
...
test('[JOURNEY-04] captures Direction B primary, detail, More, and List-first project surfaces at four breakpoints', async ({ page }) => {
  ...
  for (const viewport of VIEWPORTS) {
    ...
    await page.getByRole('button', { name: 'Capture quarterly goals', exact: true }).click();
...
test('[MORE-01] every secondary destination remains reachable by authenticated deep link', async ({ page }) => {
  ...
  for (const [path, title, reviewTab] of destinations) {
    await page.goto(path);
    await expect(page).toHaveURL(new RegExp(`${path.replace(/[/?]/g, '\\$&')}(?:\\?.*)?$`));
    await expect(page.locator('h1').filter({ hasText: title }).first()).toBeAttached();
    if (reviewTab) {
      await expect(page.getByTestId('sidebar-summary')).toHaveAttribute('aria-current', 'page');
      await expect(page.getByTestId(`review-tab-${reviewTab}`)).toHaveAttribute('aria-current', 'page');
    }
    if (path === '/settings') {
      await expect(page.getByTestId('sidebar-settings')).toHaveAttribute('aria-current', 'page');
    }
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
  }
