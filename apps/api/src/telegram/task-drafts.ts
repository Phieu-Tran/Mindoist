...
function dueLabel(draft: Pick<TelegramTaskDraft, 'dueDate' | 'dueTime'> & { endTime?: string | null }, locale: 'vi' | 'en') {
  if (!draft.dueDate) return locale === 'vi' ? 'Không' : 'None';
  const day = draft.dueDate.toISOString().slice(0, 10);
  if (!draft.dueTime) return day;
  if (draft.endTime) return `${day} ${draft.dueTime}-${draft.endTime}`;
  return `${day} ${draft.dueTime}`;
}

function startLabel(startDate: Date | null, locale: 'vi' | 'en') {
  if (!startDate) return locale === 'vi' ? 'Không' : 'None';
  return startDate.toISOString().slice(0, 10);
}

export function assertValidDateRange(startDate: Date | null, dueDate: Date | null) {
  if (startDate && dueDate && startDate > dueDate) {
    throw new TelegramLinkError('Start date must be on or before the due date', 400, 'TELEGRAM_INVALID_DATE_RANGE');
  }
}
...
function draftForm(draft: TelegramTaskDraft, locale: 'vi' | 'en') {
  if (locale === 'en') {
    return [
      ...
      `Start: ${startLabel(draft.startDate, locale)}`,
  ...
  return [
    ...
    `Bắt đầu: ${startLabel(draft.startDate, locale)}`,
...
// Single-task, batch, and edit drafts are mutually exclusive: preparing any
// one of them cancels every other pending draft so the user only ever has
// one thing waiting for confirmation at a time.
export async function cancelOtherPendingDrafts(tx: Prisma.TransactionClient, userId: string) {
  await tx.telegramTaskDraft.updateMany({ where: { userId, status: 'PENDING' }, data: { status: 'CANCELLED' } });
  await tx.telegramTaskBatchDraft.updateMany({ where: { userId, status: 'PENDING' }, data: { status: 'CANCELLED' } });
  await tx.telegramTaskEditDraft.updateMany({ where: { userId, status: 'PENDING' }, data: { status: 'CANCELLED' } });
}
...
export async function prepareTelegramTaskDraft(input: TelegramTaskDraftInput) {
  ...
  if (input.endTime && (!input.dueTime || input.endTime <= input.dueTime)) {
    throw new TelegramLinkError('End time must be after the due (start) time', 400, 'TELEGRAM_INVALID_TIME_RANGE');
  }
...
export async function confirmTelegramTaskDraft(telegramChatId: string, locale: 'vi' | 'en') {
  ...
  const result = await prisma.$transaction(async tx => {
    ...
    if (draft.dueDate && draft.dueTime && draft.endTime) {
      const owner = await tx.user.findUnique({ where: { id: userId }, select: { timeZone: true } });
      const timeZone = owner?.timeZone || 'Asia/Ho_Chi_Minh';
      const day = draft.dueDate.toISOString().slice(0, 10);
      // dueTime/endTime are wall-clock times in the user's zone. Convert them to
      // real UTC instants - stamping them with a literal "Z" stored the wall
      // time as UTC, landing every block +7h off in Asia/Ho_Chi_Minh. The draft
      // schema already guarantees endTime > dueTime on the same day.
      await tx.timeBlock.create({
        data: {
          userId,
          taskId: task.id,
          startAt: zonedDateTimeToUtc(day, draft.dueTime, timeZone),
          endAt: zonedDateTimeToUtc(day, draft.endTime, timeZone),
          timeZone,
          source: 'MANUAL',
        },
      });
