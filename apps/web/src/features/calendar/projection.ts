...
function dateTimeInZone(value: string, timeZone: string): { date: string; time: string } | null {
...
function deadlineIsCoveredByTimeBlock(
  deadline: DeadlineProjection,
  blocks: TimeBlock[],
): boolean {
  if (!deadline.time) return false;
  return blocks.some(block => {
    if (block.taskId !== deadline.taskId || block.allDay) return false;
    const blockEnd = dateTimeInZone(
      block.endAt,
      deadline.timeZone || block.timeZone || 'UTC',
    );
    return blockEnd?.date === deadline.date && blockEnd.time === deadline.time;
  });
}
...
export function calendarProjectionToEvents(
  ...
  const deadlines = liveDeadlines
    ...
    // A timed deadline normally remains as its own marker, except when a
    // planned block for the same task already ends at that exact minute. In
    // that case the marker duplicates the block's visible end edge.
    .filter(deadline => {
      if (!deadline.time) return !rangedTaskIds.has(deadline.taskId);
      return !deadlineIsCoveredByTimeBlock(deadline, projection.timeBlocks);
    })
