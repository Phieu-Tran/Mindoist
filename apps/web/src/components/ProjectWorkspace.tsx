...
export function ProjectWorkspace({
  ...
  useEffect(() => {
    if (!editingProjectName) setProjectName(project.name);
  }, [editingProjectName, project.name]);

  const saveProjectName = async () => {
    const name = projectName.trim();
    if (!onRenameProject || !name || renamingProject) return;
    ...
    try {
      ...
      setActionError(t('workspace.projectRenameError'));
    } finally {
    ...
    <section className="project-workspace" aria-labelledby="project-workspace-title">
      <header className="project-workspace-header">
        <div className="project-workspace-heading min-w-0">
          ...
          {editingProjectName ? (
            <>
              <span id="project-workspace-title" className="sr-only">{project.name}</span>
              <form className="project-workspace-rename" onSubmit={(event) => { event.preventDefault(); void saveProjectName(); }}>
                <label className="sr-only" htmlFor={`project-name-${project.id}`}>{t('workspace.projectName')}</label>
                <input
                  ...
                  autoFocus
                />
                <button type="submit" disabled={!projectName.trim() || renamingProject} aria-label={t('workspace.saveProjectName')}>
                  <Check className="h-3.5 w-3.5" aria-hidden="true" />
                </button>
                <button type="button" disabled={renamingProject} onClick={() => { setProjectName(project.name); setEditingProjectName(false); }} aria-label={t('workspace.cancel')}>
            ...
            <>
              ...
              {onRenameProject && (
                <button type="button" className="project-workspace-rename-trigger" onClick={() => { setProjectName(project.name); setEditingProjectName(true); }} aria-label={t('workspace.renameProject', { name: project.name })}>
          ...
          {topLevelTasks.length === 0 ? (
            ...
            return (
              <article key={task.id} className={cn('project-workspace-list-row', task.completedAt && 'is-completed')}>
                ...
                {task.deadline ? (
                  <DueCountdown dueDate={task.deadline.date} dueTime={task.deadline.time} completedAt={task.completedAt} compact />
                ) : <span />}
                {currentColumn && (
                  <Select
              ...
              <div className="project-column-cards">
                {columnTasks.length === 0 ? (
                  ...
                  const taskTags = task.tagIds
                    .map(id => tagMap.get(id))
                    .filter((tag): tag is TagType => Boolean(tag));
                  const hasMeta = task.priority != null || Boolean(task.deadline) || taskTags.length > 0;
                  return (
                    <article
                      key={task.id}
