...
export async function getCalendarProjection(
  ...
  const [rawTimeBlocks, tasks] = await Promise.all([
  ...
  // A time block only stores a taskId; carry its task's title (and priority,
  // project, completion) on the projection so the calendar never has to fall
  // back to a literal "time block" label for tasks it hasn't otherwise loaded
  // (e.g. completed ones, which the client's open-task list omits).
  const blockTaskIds = [...new Set(rawTimeBlocks.map(block => block.taskId))];
  const blockOwners = blockTaskIds.length
    ? await prisma.task.findMany({
        where: { id: { in: blockTaskIds }, userId, deletedAt: null },
        select: { id: true, title: true, priority: true, projectId: true, color: true, completedAt: true },
      })
    : [];
  const ownerById = new Map(blockOwners.map(owner => [owner.id, owner]));

  return {
    timeBlocks: rawTimeBlocks.map(block => {
      const owner = ownerById.get(block.taskId);
      return {
        ...block,
        title: owner?.title ?? null,
        priority: owner?.priority ?? null,
        projectId: owner?.projectId ?? null,
        colorOverride: owner?.color ?? null,
        completedAt: owner?.completedAt ?? null,
      };
    }),
