...
The pinned image exposes exactly eight reviewed Mindoist task tools:
...
- `mindoist_prepare_tasks` prepares 2–20 tasks with shared project, tags,
  color, priority and due fields. It may stage new tags only when the user
  explicitly requested their creation, and returns a complete numbered preview.
- `mindoist_confirm_tasks` atomically creates every staged tag, task and tag
  link after a later explicit confirmation; retrying confirmation is idempotent.
- `mindoist_cancel_tasks` closes the latest pending batch without creating data.
...
- `POST /internal/agent/telegram/task-batch-drafts`
- `POST /internal/agent/telegram/task-batch-drafts/confirm`
- `POST /internal/agent/telegram/task-batch-drafts/cancel`
...
3. Create one BotFather bot; configure its username on the API and token only on
   PicoClaw.
4. Build the pinned PicoClaw image; it verifies the pre-LLM guard, eight task
...
9. Prepare a numbered batch with an existing tag and one explicitly requested
   new tag. Verify the preview contains every title, then confirm once and check
   that all tasks and tags are created atomically. Repeating confirmation must
