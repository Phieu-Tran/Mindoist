...
import { isPushConfigured, sendPushToUser } from '../push/service.js';
 ...
 * and used to be delivered to the phone by pre-scheduling it there instead
 * (apps/mobile/src/notifications/local-reminders.ts), so it would fire with
 * no network at all.
 *
 * That pre-scheduled copy goes stale the moment the underlying date changes
 * on a device that isn't open to notice - the same class of bug industry
 * todo apps avoid by treating a server-timed push as the source of truth
 * instead. This worker is now that source of truth for every platform: the
 * mobile push carries the same `dedupeKey` as the phone's own locally
 * scheduled fallback, so the phone can cancel the fallback once the real
 * push lands instead of showing both.
 */
...
function taskDueAt(
  ...
  const timeZone = task.deadlineTimeZone ?? fallbackTimeZone;
  return localDateTime(date, time, timeZone);
}
  ...
  for (const countdown of countdowns) {
    ...
    try {
      const result = await sendPushToUser(countdown.userId, {
        ...
        icon: '/favicon.svg',
        dedupeKey: `countdown-due:${countdown.id}`,
      });
  ...
  for (const countdown of countdowns) {
    ...
    try {
      const result = await sendPushToUser(countdown.userId, {
        ...
        icon: '/favicon.svg',
        dedupeKey: `countdown-early:${countdown.id}`,
      });
...
export async function processDueReminders(now = new Date(), batchSize = DEFAULT_BATCH_SIZE) {
  // With neither transport configured there is nothing this worker could
  // deliver anywhere - and claiming rows it cannot send would burn them for
  // whichever transport comes online later.
  if (!isPushConfigured()) {
