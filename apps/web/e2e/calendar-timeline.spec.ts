...
test('[CALENDAR-04] selecting a time range creates a task with persisted planned time', async ({ page }) => {
  ...
  const startSlot = page.locator('.fc-timegrid-slot-lane[data-time="15:00:00"]').first();
  const finalSlot = page.locator('.fc-timegrid-slot-lane[data-time="16:30:00"]').first();
  const dayColumn = page.locator('.fc-timegrid-col[data-date]').first();
  await startSlot.evaluate(element => element.scrollIntoView({ block: 'center' }));
  const startBox = await startSlot.boundingBox();
  const finalBox = await finalSlot.boundingBox();
  const dayColumnBox = await dayColumn.boundingBox();
  expect(startBox).not.toBeNull();
  expect(finalBox).not.toBeNull();
  expect(dayColumnBox).not.toBeNull();

  const dayColumnCenter = dayColumnBox!.x + dayColumnBox!.width / 2;
  await page.mouse.move(dayColumnCenter, startBox!.y + 2);
  await page.mouse.down();
  await page.mouse.move(dayColumnCenter, finalBox!.y + finalBox!.height - 2, { steps: 12 });
  await page.mouse.up();

  const dialog = page.getByTestId('create-task-dialog');
  await expect(dialog).toBeVisible();
  await expect(dialog).toContainText(/15:00.*17:00/);
  await dialog.getByLabel('Task title').fill('Calendar focus block');
  await dialog.getByRole('button', { name: 'Create task' }).click();
  await expect(dialog).toBeHidden();

  const plannedBlock = page.locator('.calendar-time-block-event').filter({ hasText: 'Calendar focus block' });
  await expect(plannedBlock).toBeVisible();
  await expect(page.locator('.calendar-deadline-event').filter({ hasText: 'Calendar focus block' })).toHaveCount(0);
  await page.reload();
  await expect(page.locator('.calendar-time-block-event').filter({ hasText: 'Calendar focus block' })).toBeVisible();
  await expect(page.locator('.calendar-deadline-event').filter({ hasText: 'Calendar focus block' })).toHaveCount(0);

  await page.locator('.calendar-time-block-event').filter({ hasText: 'Calendar focus block' }).click();
  const plannedTimeEditor = page.getByRole('region', { name: 'Planned time' });
  await expect(plannedTimeEditor).toBeVisible();
  await expect(plannedTimeEditor.getByRole('button', { name: /^Edit.*15:00.*17:00/ })).toBeVisible();

  await page.getByTestId('detail-title-complete').click();
  await expect(plannedBlock).toHaveClass(/calendar-task-event-completed/);
  await expect(plannedBlock.locator('.calendar-event-card-title')).toHaveCSS('text-decoration-line', 'line-through');
  expect(pageErrors).toEqual([]);
});
