...
import { MINUTES_PER_DAY, SLOT_MINUTES, dayKey, snapMinutes } from './time-grid';
      ...
      {blocks.map(block => {
        ...
        const style = {
          ...
          height: resizing?.blockId === blockId
            ? Math.max(18, ((resizing.end - (item.start.getHours() * 60 + item.start.getMinutes())) / SLOT_MINUTES) * 20)
            : block.height,
