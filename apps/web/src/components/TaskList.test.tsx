...
const makeTask = (id: string, title: string, completedAt: string | null = null, overrides: LegacyTaskOverrides = {}): Task => ({
  id, userId: 'u1', projectId: null, projectColumnId: null, sectionId: null, parentId: null,
  title, description: null, color: null, priority: 0, deadline: null,
  startDate: null, estimateMin: null, rrule: null, pomodoroCount: 0, completedAt, sortOrder: 0,
...
describe('TaskList', () => {
  ...
  it('inherits the project color as the list-row identity', () => {
    renderList(
      [makeTask('1', 'Project task', null, { projectId: 'project-1', color: null })],
      false,
      null,
      undefined,
      { projects: [{ id: 'project-1', name: 'Jade launch', color: 'jade' } as Project] },
    );
    const taskEl = screen.getByTestId('task-1');
    expect(taskEl.className).toContain('task-color-jade');
    expect(taskEl.className).not.toContain('border-l-');
    expect(taskEl.className).not.toContain('focus-within:bg-card');
  });
  ...
  it('uses the shared controlled selection used by keyboard shortcuts', () => {
    const onBulkSelectionChange = vi.fn();
    renderList(
      [makeTask('1', 'First'), makeTask('2', 'Second')],
      false,
      null,
      'inbox',
      {
        onBulkComplete: vi.fn(),
        bulkSelectedIds: new Set(['1', '2']),
    ...
    fireEvent.click(screen.getByTestId('task-select-1'));
    expect([...onBulkSelectionChange.mock.calls[0][0]]).toEqual(['2']);
  });
  ...
  it('highlights a selected task that has no color with a background tint', () => {
    ...
    expect(taskEl.className).toContain('shadow-sm');
    expect(taskEl.className).not.toContain('ring-1');
  ...
  it('keeps the task color tint when a colored task is selected', () => {
    ...
    expect(taskEl.className).toContain('shadow-sm');
    expect(taskEl.className).not.toContain('ring-2');
  });
});
