...
export function SummaryDashboard({ tasks, projects, loading, error, onRetry, onSelectTask }: Props) {
  ...
  const periodTasks = useMemo(() => {
    ...
    return filteredTasks.filter(task => {
      const date = task.completedAt || task.dueDate || task.createdAt;
  ...
  const stats = useMemo(() => {
    ...
    const overdue = periodTasks.filter(task => (
      !task.completedAt && task.dueDate && task.dueDate.slice(0, 10) < today
    )).length;
      ...
      {tasks.length === 0 ? (
        ...
        <>
          ...
          <div className="summary-grid">
                    ...
                    <li key={task.id}>
                      <button type="button" onClick={() => onSelectTask?.(task)}>
                        ...
                        <span className="summary-agenda-meta">
                          {[task.dueDate?.slice(0, 10), task.priority != null ? `P${task.priority}` : null].filter(Boolean).join(' · ')}
                        </span>
            ...
            <SummaryCalendar tasks={filteredTasks} onSelectTask={onSelectTask} />
          </div>
        </>
      )}
    </section>
