...
import {
  cancelTelegramTaskEditDraft,
  confirmTelegramTaskEditDraft,
  prepareTelegramTaskEditDraft,
} from './task-edit.js';
import {
  listTelegramTasks,
...
const taskDraftSchema = z.object({
  ...
  if (value.endTime && !value.dueTime) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['endTime'], message: 'endTime requires dueTime' });
  }
});
...
const taskEditDraftSchema = z.object({
  telegramChatId: telegramId,
  locale,
...
export async function telegramRoutes(app: FastifyInstance) {
  ...
  app.post('/internal/agent/telegram/task-edit-drafts', { preHandler: requireAgentAuth }, async (request, reply) => {
    const parsed = taskEditDraftSchema.safeParse(request.body);
    if (!parsed.success) {
      return reply.status(400).send({ success: false, error: 'Invalid task edit draft request' });
    }
    try {
      const data = await prepareTelegramTaskEditDraft(parsed.data);
      request.log.info({ event: 'telegram_task_edit_draft_prepared', state: data.state });
      return reply.status(201).send({ success: true, data });
    } catch (error) {
      return sendTelegramError(error, reply);
    }
  ...
  app.post('/internal/agent/telegram/task-edit-drafts/confirm', { preHandler: requireAgentAuth }, async (request, reply) => {
    const parsed = taskDraftActionSchema.safeParse(request.body);
    if (!parsed.success) return reply.status(400).send({ success: false, error: 'Invalid task edit draft request' });
    try {
      const data = await confirmTelegramTaskEditDraft(parsed.data.telegramChatId, parsed.data.locale);
      request.log.info({ event: 'telegram_task_edit_draft_confirmed' });
      return reply.send({ success: true, data });
    } catch (error) {
      return sendTelegramError(error, reply);
    }
  ...
  app.post('/internal/agent/telegram/task-edit-drafts/cancel', { preHandler: requireAgentAuth }, async (request, reply) => {
    const parsed = taskDraftActionSchema.safeParse(request.body);
    if (!parsed.success) return reply.status(400).send({ success: false, error: 'Invalid task edit draft request' });
    try {
      const data = await cancelTelegramTaskEditDraft(parsed.data.telegramChatId, parsed.data.locale);
      request.log.info({ event: 'telegram_task_edit_draft_cancelled' });
      return reply.send({ success: true, data });
    } catch (error) {
      return sendTelegramError(error, reply);
    }
