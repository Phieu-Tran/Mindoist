...
export function countVisibleTasks(tasks: Task[], view: SidebarView, now = new Date()): number {
  ...
  if (view === 'inbox') {
    return topLevel.filter(task => !task.projectId && !task.deadline && !task.completedAt && !task.deletedAt).length;
  }
...
export function selectCalendarTaskViews(
  ...
  const todayTasks = calendarTasks.filter(task => {
    if (task.completedAt || task.deletedAt || !task.deadline) return false;
    const due = task.deadline.date;
    const start = task.startDate?.slice(0, 10) ?? due;
    return start <= today && today <= due;
  });
  const backlogTasks = calendarTasks.filter(task => !task.deadline && !task.completedAt && !task.deletedAt);
