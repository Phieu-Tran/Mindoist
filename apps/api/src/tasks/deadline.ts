...
export function applyDeadlineInput(
  ...
  // Compatibility window: old clients and Google sync still consume these.
  data.dueDate =
    deadline.time && deadline.timeZone
      ? zonedDateTimeToUtc(deadline.date, deadline.time, deadline.timeZone)
      : deadlineDate;
  data.dueTime = deadline.time ?? null;
}

export function mirrorLegacyDeadline(
  data: MutableTaskData,
  dueDate: Date | null,
  dueTime: string | null,
  existingTimeZone: string | null = null,
): void {
  data.deadlineDate = dueDate;
  data.deadlineTime = dueDate ? dueTime : null;
  data.deadlineTimeZone = dueDate && dueTime ? existingTimeZone : null;
}
