...
export default function App() {
  ...
  const [calendarDraft, setCalendarDraft] = useState<CalendarTaskSlot | null>(null);
  ...
  const handleCalendarCreate = useCallback((slot: CalendarTaskSlot) => {
    setCalendarDraft(slot);
  }, []);

  const handleCalendarSubmit = useCallback(async (title: string) => {
    if (!calendarDraft) return;
    const isTimed = !calendarDraft.allDay && Boolean(calendarDraft.startAt && calendarDraft.endAt);
    const durationMin = isTimed
      ? Math.max(1, Math.round(
          (new Date(calendarDraft.endAt!).getTime() - new Date(calendarDraft.startAt!).getTime()) / 60_000,
        ))
      : null;
    const task = await addTask({
      title,
      dueDate: isTimed ? calendarDraft.endDate || calendarDraft.date : calendarDraft.date,
      dueTime: isTimed ? calendarDraft.endTime || null : null,
      durationMin,
    } as CreateTaskRequest);
    ...
    if (isTimed) {
      try {
        await createTimeBlock({
          taskId: task.id,
          startAt: calendarDraft.startAt!,
          endAt: calendarDraft.endAt!,
  ...
  }, [addTask, calendarDraft, deleteTask, refetchTasks]);
