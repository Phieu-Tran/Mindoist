...
function yesterday() {
  return addDatabaseDateDays(today(), -1);
}
...
describe('Task startDate (date range)', () => {
  it('POST /tasks creates a task with startDate + deadline', async () => {
    const res = await app.inject({
      method: 'POST',
      url: '/tasks',
      headers: { authorization: `Bearer ${userToken}` },
      payload: { title: 'Ranged', startDate: '2026-07-20', deadline: { date: '2026-07-25' } },
    });
    expect(res.statusCode).toBe(201);
    const body = JSON.parse(res.body);
    expect(body.data.startDate).toContain('2026-07-20');
    expect(body.data.deadline).toEqual({ date: '2026-07-25' });
  });
  ...
  it('POST /tasks creates a task without startDate (null)', async () => {
    const res = await app.inject({
      ...
      url: '/tasks',
      headers: { authorization: `Bearer ${userToken}` },
      payload: { title: 'No range', deadline: { date: '2026-07-25' } },
    });
    expect(res.statusCode).toBe(201);
    expect(JSON.parse(res.body).data.startDate).toBeNull();
  });

  it('POST /tasks rejects startDate after deadline', async () => {
    const res = await app.inject({
      method: 'POST',
      url: '/tasks',
      headers: { authorization: `Bearer ${userToken}` },
      payload: { title: 'Bad range', startDate: '2026-07-26', deadline: { date: '2026-07-25' } },
    });
    expect(res.statusCode).toBe(400);
  });
  ...
  it('PATCH /tasks/:id sets and clears startDate', async () => {
    const t = await prisma.task.create({
      data: { userId, title: 'Patch range', deadlineDate: new Date('2026-07-25') },
    });
  ...
  it('PATCH /tasks/:id rejects startDate after the existing deadline', async () => {
    const t = await prisma.task.create({
      data: { userId, title: 'Patch bad range', deadlineDate: new Date('2026-07-25') },
    });
    const res = await app.inject({
      method: 'PATCH',
      url: `/tasks/${t.id}`,
  ...
  it('PATCH /tasks/:id rejects moving deadline before the existing startDate', async () => {
    const t = await prisma.task.create({
      data: { userId, title: 'Patch bad due', startDate: new Date('2026-07-20'), deadlineDate: new Date('2026-07-25') },
    });
    const res = await app.inject({
      method: 'PATCH',
      url: `/tasks/${t.id}`,
      headers: { authorization: `Bearer ${userToken}` },
      payload: { deadline: { date: '2026-07-10' } },
    });
    expect(res.statusCode).toBe(400);
  });
});
...
describe('Task smart list filters', () => {
  it('filter=inbox excludes completed tasks (Inbox is an active worklist, not everything)', async () => {
    ...
    await prisma.task.create({
      data: { userId, title: 'Open, has due date', deadlineDate: tomorrow() },
    });
  ...
  it('filter=today returns today tasks', async () => {
    await prisma.task.create({
      data: { userId, title: 'Today Task', deadlineDate: today() },
    });
    await prisma.task.create({
      data: { userId, title: 'Yesterday Task', deadlineDate: yesterday() },
    });
  ...
  it('filter=upcoming returns future tasks', async () => {
    await prisma.task.create({
      data: { userId, title: 'Tomorrow', deadlineDate: tomorrow() },
    });
    await prisma.task.create({
      data: { userId, title: 'Yesterday', deadlineDate: yesterday() },
    });
  ...
  it('filter=upcoming excludes tasks due more than 7 days out (Next 7 Days is not just "any future task")', async () => {
    await prisma.task.create({
      data: { userId, title: 'In 3 days', deadlineDate: (() => { const d = new Date(); d.setDate(d.getDate() + 3); return d; })() },
    });
    await prisma.task.create({
      data: { userId, title: 'In 10 days', deadlineDate: (() => { const d = new Date(); d.setDate(d.getDate() + 10); return d; })() },
    });
  ...
  it('filter=overdue returns past tasks', async () => {
    await prisma.task.create({
      data: { userId, title: 'Late', deadlineDate: yesterday() },
    });
    await prisma.task.create({
      data: { userId, title: 'Future', deadlineDate: tomorrow() },
    });
  ...
  it('filter=today/upcoming/overdue still include subtasks (no deadline of their own) so the frontend can compute accurate done/total counts', async () => {
    const parentToday = await prisma.task.create({ data: { userId, title: 'Parent due today', deadlineDate: today() } });
    ...
    const parentUpcoming = await prisma.task.create({ data: { userId, title: 'Parent due tomorrow', deadlineDate: tomorrow() } });
    await prisma.task.create({ data: { userId, title: 'Upcoming subtask', parentId: parentUpcoming.id } });

    const parentOverdue = await prisma.task.create({ data: { userId, title: 'Parent overdue', deadlineDate: yesterday() } });
  ...
  it('complete task with DAILY rrule creates next occurrence +1 day', async () => {
    const t = await prisma.task.create({
      data: {
        ...
        title: 'Daily Standup',
        projectId,
        deadlineDate: new Date('2026-07-18T00:00:00Z'),
        rrule: 'FREQ=DAILY',
      },
    });
    const res = await app.inject({
...
describe('GET /task-counts', () => {
  it('excludes subtasks from every count so sidebar badges match the visible list', async () => {
    const parent = await prisma.task.create({ data: { userId, title: 'Parent', deadlineDate: today() } });
    ...
    await prisma.task.create({ data: { userId, title: 'Sub today', parentId: parent.id, deadlineDate: today() } });
