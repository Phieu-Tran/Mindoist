...
test('[PROJECTS-01] project workspace defaults to List, persists Board opt-in, and filters root task cards', async ({ page }) => {
  ...
  await page.getByTestId('global-quick-capture-input').fill('Portfolio review tomorrow p2');
  ...
  await page.keyboard.press('Control+k');
  await page.getByTestId('global-quick-capture-input').fill('Mock interview today p1');
  await page.getByTestId('global-quick-capture-submit').click();
  await expect(page.getByTestId('global-quick-capture')).toBeHidden();
...
test('[PROJECTS-03] project name can be edited from the workspace heading', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await register(page);
  await createJobProject(page, 'Rename me');

  await page.getByRole('button', { name: 'Rename Rename me' }).click();
  await page.getByRole('textbox', { name: 'Project name' }).fill('Launch command');
  mkdirSync(EVIDENCE_DIR, { recursive: true });
  await page.screenshot({ path: `${EVIDENCE_DIR}/project-rename-1440.png`, fullPage: true });
  await page.getByRole('button', { name: 'Save project name' }).click();

  await expect(page.getByRole('heading', { name: 'Launch command' })).toBeVisible();
  await expect(page.getByTestId(/sidebar-project-/).filter({ hasText: 'Launch command' })).toBeVisible();
});
