...
import { Menu, X, Inbox, CalendarDays, FolderOpen, MoreHorizontal, Plus } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-screen bg-background overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        >
          {isDesktop && <h1 className="sr-only">{viewTitle}</h1>}
          {children}
        </main>
      </div>
      ...
      {showMobileBottomNav && (
        ...
        >
          ...
          <BottomNavItem
            icon={Inbox}
            label={t('tasks:sidebar.inbox')}
            active={sidebarView === 'all' && !currentProjectId && !currentTagId}
...
function BottomNavItem({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 py-1 text-[10px] transition-colors cursor-pointer",
