...
### Added
- Summary now has project-aware weekly (Monday–Sunday) and monthly task ledgers with daily groups, period totals, and one-click Markdown handoff to Obsidian through the official Obsidian URI flow (no plugin required).
- Weekly and monthly Obsidian notes can be previewed and copied before sending, including frontmatter and task checkboxes.
- Settings → Integrations now includes per-account, per-device Obsidian vault, vault-relative folder, and separate weekly/monthly filename templates with validation and destination previews.

### Fixed
- Opening a task from Summary now keeps the `/review` workspace and selected month/list in place while showing Task Inspector, instead of navigating away to the standalone task route.
- Completed tasks now keep a visible strike-through across Calendar time blocks, all-day chips, deadlines, and month cells; the time grid also uses slightly taller slots, wider day columns, and more readable time labels.
- Global Quick Capture now uses Mindoist-styled, localized date and time pickers instead of the browser's native calendar, with a clearer responsive planning row and safer keyboard/touch targets.
