...
import { effectiveTaskColor, taskColorClass } from '@/lib/task-colors';
...
const CALENDAR_TASK_DRAG_TYPE = 'application/x-panel-task';
const TASK_DURATION_DRAG_TYPE = 'application/x-task-duration';
...
function getDueMeta(task: Task, t: (k: string, opts?: any) => string): { label: string; className: string } | null {
  ...
  const dueDate = task.deadline.date;
...
function TaskMeta({
  ...
  const countdownDate = task.deadline?.date ?? null;
  const countdownTime = task.deadline?.time ?? null;
...
function TaskRow({
  ...
  return (
    ...
    <div
      ...
      className={cn(
        "task-row-density group relative grid grid-cols-[2.75rem_minmax(0,1fr)] px-1 transition-colors",
        hasMeta && "task-row-density-has-meta",
        "hover:shadow-sm focus-within:shadow-sm",
        !identityColor && "hover:bg-card focus-within:bg-card",
        identityColor && "bg-[var(--task-color-bg)]",
        taskColorClass(identityColor),
        isSelected && (identityColor
          ? "shadow-sm"
          : "bg-sidebar-accent/80 shadow-sm"),
    ...
    >
      ...
      <div className="flex items-start justify-center pt-[var(--density-task-padding)]">
        {onRestore ? (
          <Button
            type="button"
            variant="ghost"
      ...
      <button
        ...
        className={cn(
          "min-w-0 rounded-md py-[var(--density-task-padding)] pr-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
          isBulkEnabled && "pr-9"
        )}
      >
        <span
...
export function TaskList({ tasks, loading, error, onToggle, onSelect, onBulkComplete, onBulkDelete, onRetry, onMakeSubtask, onRestore, view, selectedTaskId, bulkSelectedIds: controlledBulkSelectedIds, onBulkSelectionChange, tags = [], projects = [] }: Props) {
  const { t } = useTranslation('tasks');
  const [animatingIds, setAnimatingIds] = useState<Set<string>>(new Set());
  const [internalBulkSelectedIds, setInternalBulkSelectedIds] = useState<Set<string>>(new Set());
  const bulkSelectedIds = controlledBulkSelectedIds ?? internalBulkSelectedIds;
  ...
  const handleRowDragStart = useCallback((task: Task, event: React.DragEvent) => {
    ...
    event.dataTransfer.setData(CALENDAR_TASK_DRAG_TYPE, task.id);
    event.dataTransfer.setData(TASK_DURATION_DRAG_TYPE, String(task.estimateMin ?? 60));
  ...
  const displayTasks = useMemo(() => {
    ...
    if (view === 'inbox') return topLevel.filter(task => !task.projectId && !task.deadline && !task.completedAt && !task.deletedAt);
    if (view === 'today') {
      const now = new Date();
  ...
  }, [controlledBulkSelectedIds, onBulkSelectionChange]);
  ...
  const toggleBulkSelection = useCallback((task: Task) => {
    ...
    commitBulkSelection(next);
  }, [bulkSelectedIds, commitBulkSelection]);
  ...
  const groupedTasks = useMemo(() => {
    ...
    for (const task of displayTasks) {
      ...
      if (view === 'completed') {
        ...
        key = task.deadline?.date || 'no-date';
      }
    ...
    <div data-testid="task-list" className="flex-1">
      {error && (
        ...
        >
          <span>{error}</span>
          {onRetry && (
            <Button variant="outline" size="sm" data-testid="retry-btn" onClick={onRetry} className="min-h-11 text-xs">
              {t('list.retry')}
            </Button>
          )}
        </div>
