...
import { applyDeadlineInput } from './deadline.js';

async function syncTaskCalendar(userId: string, taskId: string): Promise<void> {
...
export async function taskRoutes(app: FastifyInstance) {
  ...
  app.get('/tasks', { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (filter) {
      ...
      switch (filter) {
        case 'inbox':
          ...
          where.AND = [...(where.AND ?? []), visibleAfterSnooze];
          break;
        case 'today':
          where.completedAt = null;
          where.deadlineDate = todayDate;
          where.AND = [...(where.AND ?? []), visibleAfterSnooze];
          break;
        case 'upcoming':
          where.completedAt = null;
          where.deadlineDate = { gt: todayDate, lte: next7End };
          where.AND = [...(where.AND ?? []), visibleAfterSnooze];
          break;
        case 'overdue':
          where.completedAt = null;
          where.deadlineDate = { lt: todayDate };
          where.AND = [...(where.AND ?? []), visibleAfterSnooze];
          break;
        case 'completed':
    ...
    // array. The deadline range (today/upcoming/overdue) must move into the
    // same branch too — subtasks are created without their own deadline, so
    // leaving the deadline as a top-level AND condition would still exclude every
    // subtask from those three views.
    if (where.completedAt === null) {
      const existingOr = where.OR;
      const existingAnd = where.AND;
      const existingDeadlineDate = where.deadlineDate;
      delete where.completedAt;
      delete where.deadlineDate;
      const viewBranch: any = { completedAt: null };
      if (existingDeadlineDate) viewBranch.deadlineDate = existingDeadlineDate;
      if (existingOr) viewBranch.OR = existingOr;
      if (existingAnd) viewBranch.AND = existingAnd;
      where.OR = [
        { parentId: { not: null } },
        viewBranch,
      ];
  ...
  app.post('/tasks', { preHandler: requireAuth }, async (request, reply) => {
    ...
    const { startDate, deadline, estimateMin, snoozedUntil, ...rest } = parsed.data;
    ...
    if (snoozedUntil !== undefined) data.snoozedUntil = snoozedUntil ? new Date(snoozedUntil) : null;

    // A start date is a range decoration and cannot follow the deadline day.
    ...
    // Calendar exports are driven by explicit TimeBlocks; a deadline alone is
    // a point marker and must not create a Google event.
    syncTaskCalendar(request.auth!.sub, task.id).catch(() => {});
  ...
  app.patch('/tasks/:id', { preHandler: requireAuth }, async (request, reply) => {
    ...
    const { startDate, deadline, estimateMin, snoozedUntil, tagIds, ...rest } = parsed.data;
    const data: any = { ...rest };
    if (estimateMin !== undefined) data.estimateMin = estimateMin;
    const scheduleChanged = deadline !== undefined;
    if (deadline !== undefined) applyDeadlineInput(data, deadline);
    if (scheduleChanged) {
