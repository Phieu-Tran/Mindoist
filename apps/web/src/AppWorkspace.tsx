...
import { loadObsidianSettings, saveObsidianSettings, type ObsidianSettings } from './lib/obsidian-settings';
...
function AuthenticatedWorkspace({ user, setPassword, logout }: AuthenticatedWorkspaceProps) {
  ...
  const [obsidianSettings, setObsidianSettings] = useState<ObsidianSettings>(() => loadObsidianSettings(user.id));
  ...
  const handleSaveObsidianSettings = useCallback((settings: ObsidianSettings) => {
    setObsidianSettings(saveObsidianSettings(user.id, settings));
  }, [user.id]);

  const handleOpenObsidianSettings = useCallback(() => {
    setSettingsInitialCategory('integrations');
