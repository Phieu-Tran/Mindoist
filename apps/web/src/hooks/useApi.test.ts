...
describe('useApi hooks', () => {
  ...
  describe('useProjectColumns', () => {
    ...
    it('fetches columns and keeps create, update, and delete changes in local state', async () => {
      ...
      await act(async () => { await new Promise(resolve => setTimeout(resolve, 0)); });
      expect(result.current.columns).toEqual([firstColumn]);
      ...
      expect(result.current.columns).toEqual([firstColumn, secondColumn]);
      ...
      expect(result.current.columns[1].name).toBe('Complete');
