...
const TELEGRAM_QUERY_A = `${Date.now()}1`;
const TELEGRAM_QUERY_B = `${Date.now()}2`;
...
describe('Telegram read-only task queries', () => {
  ...
  beforeAll(async () => {
    ...
    const staleUsers = await prisma.user.findMany({
      where: { email: { in: ['telegram-query-a@example.com', 'telegram-query-b@example.com'] } },
      select: { id: true },
    });
  ...
  beforeEach(async () => {
    ...
    await prisma.telegramConnection.createMany({
      data: [
        { userId: userA.user.id, telegramUserId: TELEGRAM_QUERY_A, telegramChatId: TELEGRAM_QUERY_A },
        { userId: userB.user.id, telegramUserId: TELEGRAM_QUERY_B, telegramChatId: TELEGRAM_QUERY_B },
      ],
    });
  });
  ...
  function query(path: 'list' | 'summary', payload: Record<string, unknown>) {
    return app.inject({
      ...
      payload: { telegramChatId: TELEGRAM_QUERY_A, locale: 'vi', ...payload },
    });
  }
