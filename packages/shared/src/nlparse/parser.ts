...
export interface ParsedQuickAdd {
  ...
  estimateMin?: number;
  projectId?: string;
  reminderOffsetMin?: number; // minutes before the deadline the reminder fires
}
...
function parseEn(input: string, now: Date): ParsedQuickAdd {
  ...
  if (dueDate) result.deadline = { date: dueDate, ...(dueTime ? { time: dueTime } : {}) };
  if (priority) result.priority = priority;
  if (durationMin) result.estimateMin = durationMin;
...
function parseVi(input: string, now: Date): ParsedQuickAdd {
  ...
  title = cleanTitle(title);
  const result: ParsedQuickAdd = { title };
  if (dateMatch || timeMatch) result.deadline = {
    date: dateMatch?.date ?? fmtDate(now),
    ...(timeMatch ? { time: timeMatch.time } : {}),
  };
  if (priority) result.priority = priority;
  if (durationMin) result.estimateMin = durationMin;
  if (projectId) result.projectId = projectId;
