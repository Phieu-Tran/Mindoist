...
describe('calendarProjectionToEvents', () => {
  ...
  it('marks a completed task time block for struck-through calendar styling', () => {
    const [timeBlock] = calendarProjectionToEvents(
      identityProjection,
      [{ ...identityTask, completedAt: '2026-08-11T04:45:00.000Z' }],
      [],
    );

    expect(timeBlock.extendedProps?.kind).toBe('timeBlock');
    expect(timeBlock.classNames).toContain('calendar-task-event-completed');
  });
...
describe('calendar task range projection', () => {
  ...
  it('hides a deadline marker when a planned block already ends at that exact minute', () => {
    const projection: CalendarProjection = {
      timeBlocks: [{
        ...identityProjection.timeBlocks[0],
        taskId: 'task-1',
        startAt: '2026-08-11T04:00:00.000Z',
        endAt: '2026-08-11T04:45:00.000Z',
        timeZone: 'Asia/Ho_Chi_Minh',
      }],
    ...
    const events = calendarProjectionToEvents(
      projection,
      [makeTask({ dueDate: '2026-08-11', dueTime: '11:45', startDate: null, durationMin: 45 })],
      [],
    );

    expect(events).toHaveLength(1);
    expect(events[0].extendedProps?.kind).toBe('timeBlock');
    expect(events[0].start).toBe('2026-08-11T04:00:00.000Z');
    expect(events[0].end).toBe('2026-08-11T04:45:00.000Z');
  });
