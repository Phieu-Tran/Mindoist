...
function calendarEventLabel(event: EventApi): string | null {
  ...
  if (event.extendedProps.source === 'google') {
    const date = event.start
      ...
      : '';
    return `${event.title} · Google Calendar · Read only${date ? ` · ${date}` : ''}`;
  }
...
function refreshEventAccessibility(element: HTMLElement, event: EventApi): void {
...
const CUSTOM_VIEWS = {
  timeGrid3Day: { type: 'timeGrid', duration: { days: 3 } },
  timeGrid5Day: { type: 'timeGrid', duration: { days: 5 } },
} as const;
...
export function CalendarView({ tasks, todayTasks = [], backlogTasks = [], projects = [], gcalEvents = [], countdowns = [], onSelectTask, onCreateTask, onUpdateTask, workHoursPerDay = 8 }: Props) {
  ...
  const loadProjection = useCallback(async (from: Date, to: Date) => {
    ...
    try {
      ...
      setProjectionError(
        cause instanceof Error ? cause.message : 'Unable to load calendar projection',
      );
    }
  }, []);
    ...
    try {
      if (featureFlags.calendarV2 && kind === 'timeBlock') {
        ...
        setProjection(previous => previous
          ...
          : previous);
        refreshEventAccessibility(info.el, info.event);
        return;
      }
      ...
      info.revert();
      setProjectionError('Calendar change could not be saved. The event was restored.');
    }
  }, [onUpdateTask]);
  ...
  const handleEventResize = useCallback(async (info: EventResizeDoneArg) => {
    ...
    try {
      ...
      info.revert();
      setProjectionError('Calendar resize could not be saved. The event was restored.');
    }
  }, []);
  ...
  const handleCalendarDrop = useCallback(async (info: DropArg) => {
    ...
    if (featureFlags.calendarV2) {
      ...
      try {
        ...
        setProjectionError('Planned time could not be created. Try dropping the task again.');
      }
      return;
    }
  ...
  const handleEventDidMount = useCallback((info: EventMountArg) => {
    ...
    const label = calendarEventLabel(info.event)!;
