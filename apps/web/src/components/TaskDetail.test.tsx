...
const makeTask = (id: string, overrides: Partial<Task> = {}): Task => ({
  ...
  title: 'Test Task', description: null, color: null, priority: 0, dueDate: null, dueTime: null,
...
describe('TaskInspector', () => {
  ...
  const renderInspector = (task?: Partial<Task>) =>
    render(
  ...
  it('D4: title, then description, then the properties row (color/dates)', () => {
    ...
    const startDate = screen.getByTestId('detail-start-date');
  ...
  it('displays due date', () => {
    ...
    const date = screen.getByTestId('detail-due-date');
    expect(date).toHaveTextContent('25/07/2026');
    expect(date.querySelector('span[aria-hidden="true"]')).toBeNull();
  });

  it('displays due time', () => {
    renderInspector({ dueTime: '09:00' });
    expect(screen.getByTestId('detail-due-time')).toHaveTextContent('09:00');
  });

  it('displays start date when set', () => {
    renderInspector({ startDate: '2026-07-20T00:00:00.000Z', dueDate: '2026-07-25T00:00:00.000Z' });
    expect(screen.getByTestId('detail-start-date')).toHaveTextContent('20/07/2026');
    expect(screen.getByTestId('detail-due-date')).toHaveTextContent('20/07/2026');
    expect(screen.getByTestId('detail-due-date')).toHaveTextContent('25/07/2026');
    expect(screen.getByTestId('detail-due-date').querySelector('span[aria-hidden="true"]')).not.toBeNull();
    expect(screen.queryByTestId('detail-due-date-today')).not.toBeInTheDocument();
  });
  ...
  it('updates due date immediately when a day is picked (no blur/Enter needed)', () => {
    renderInspector({ dueDate: '2026-07-25T00:00:00.000Z' });
    fireEvent.click(screen.getByTestId('detail-due-date'));
    fireEvent.click(screen.getByRole('button', { name: '15' }));
    expect(screen.getByTestId('detail-due-date')).toHaveTextContent('15/07/2026');
  ...
  it('saves a selected task color', async () => {
    ...
    expect(screen.getByTestId('detail-color')).toHaveClass('task-color-sky');
    fireEvent.click(screen.getByTestId('detail-save'));
    await waitFor(() => {
  ...
  it('offers an icon Save in the header that submits the same form', () => {
    renderInspector({ title: 'Original' });
    const titleInput = screen.getByTestId('detail-title');
    fireEvent.change(titleInput, { target: { value: 'Updated from header' } });

    const saveElements = document.querySelectorAll('[data-testid*="save"]');
    expect(saveElements).toHaveLength(2);
    expect(screen.getByTestId('detail-save-header')).toHaveAttribute('form');
    fireEvent.click(screen.getByTestId('detail-save-header'));

    expect(onSave).toHaveBeenCalledWith('1', expect.objectContaining({ title: 'Updated from header' }));
  });
