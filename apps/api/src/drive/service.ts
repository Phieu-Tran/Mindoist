    ...
    for (const t of data.tasks as any[]) {
      ...
      const created = await tx.task.create({
        data: {
          ...
          // Backups may be from either schema generation. Prefer canonical
          // deadline fields while accepting legacy values from older archives.
          startDate: t.startDate ? new Date(t.startDate) : null,
          deadlineDate: (t.deadlineDate ?? t.dueDate) ? new Date(t.deadlineDate ?? t.dueDate) : null,
          deadlineTime: t.deadlineTime ?? t.dueTime ?? null,
          deadlineTimeZone: t.deadlineTimeZone ?? null,
          estimateMin: t.estimateMin ?? t.durationMin ?? null,
          rrule: t.rrule ?? null,
