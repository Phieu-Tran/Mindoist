...
interface Props {
  ...
  onCreateTask: (slot: CalendarTaskSlot) => void;
...
function taskToEvent(task: Task): EventInput | null {
  ...
  let start = hasTime ? `${dateStr}T${dueTime}:00` : dateStr;
  let end: string | undefined;
  if (hasTime && task.durationMin && task.durationMin > 0) {
    const deadline = new Date(`${dateStr}T${dueTime}:00`);
    const plannedStart = new Date(deadline.getTime() - task.durationMin * 60 * 1_000);
    start = `${dateToLocalISO(plannedStart)}T${timeFromDate(plannedStart)}:00`;
    end = `${dateStr}T${dueTime}:00`;
  }
...
export function CalendarView({ tasks, todayTasks = [], backlogTasks = [], projects = [], gcalEvents = [], countdowns = [], onSelectTask, onCreateTask, onUpdateTask, workHoursPerDay = 8 }: Props) {
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      if (view) void loadProjection(view.activeStart, view.activeEnd);
    };
    window.addEventListener(CALENDAR_PROJECTION_REFRESH_EVENT, refresh);
    return () => window.removeEventListener(CALENDAR_PROJECTION_REFRESH_EVENT, refresh);
  }, [loadProjection]);
  ...
  const handleDateSelect = useCallback((info: DateSelectArg) => {
    const date = dateToLocalISO(info.start);
    if (info.allDay) {
      onCreateTask({ date, allDay: true });
      return;
    }
    onCreateTask({
      date,
      endDate: dateToLocalISO(info.end),
