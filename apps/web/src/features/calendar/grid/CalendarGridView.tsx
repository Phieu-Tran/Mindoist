...
import { dayKey, snapMinutes, threeDayDates, weekDates, workWeekDates } from './time-grid';
...
function layoutForDay(day: Date, items: CalendarItem[]): PositionedBlock[] {
  ...
  return layoutTimedItems(timed, { startHour: 6, slotHeight: 20, minHeight: 24 });
}
...
export function CalendarGridView({
  ...
  return (
    <div className="mindoist-calendar-grid" data-calendar-engine="grid" style={{ '--grid-days': days.length } as CSSProperties}>
      ...
      <div className="mindoist-calendar-all-day-row">
        ...
        {days.map(day => {
          ...
          return <div key={key} className={`mindoist-calendar-grid-all-day${key === today ? ' is-today' : ''}`}>{allDayItems.slice(0, 3).map(item => {
            ...
            const className = `mindoist-calendar-grid-chip is-${item.kind}`;
