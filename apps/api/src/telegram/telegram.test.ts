...
describe('Telegram account linking', () => {
  ...
  function authorizeMessage(
    ...
    telegramMessageDate = Math.floor(Date.now() / 1000),
  ) {
    return app.inject({
      method: 'POST',
  ...
  it('accepts each fresh Telegram message once and rejects a polling retry', async () => {
    const { code } = await createChallenge();
    expect((await consume(code, '10101')).statusCode).toBe(200);

    const first = await authorizeMessage('10101', '501');
    const duplicate = await authorizeMessage('10101', '501');

    expect(first.statusCode).toBe(200);
    expect(first.json().data).toMatchObject({ userId: userA.user.id, accepted: true });
    ...
    expect(await prisma.telegramInboundMessage.count({
      where: { telegramChatId: '10101', telegramMessageId: '501' },
    })).toBe(1);
  });

  it('rejects stale backlog before LLM processing and records the decision', async () => {
    const { code } = await createChallenge();
    expect((await consume(code, '10102')).statusCode).toBe(200);

    const stale = await authorizeMessage('10102', '601', Math.floor(Date.now() / 1000) - 4 * 60);

    expect(stale.statusCode).toBe(200);
    expect(stale.json().data).toMatchObject({ accepted: false, reason: 'stale' });
    expect(await prisma.telegramInboundMessage.findUnique({
      where: {
        telegramChatId_telegramMessageId: {
          telegramChatId: '10102',
  ...
  it('claims a concurrent duplicate exactly once', async () => {
    const { code } = await createChallenge();
    expect((await consume(code, '10103')).statusCode).toBe(200);

    const responses = await Promise.all([
      authorizeMessage('10103', '701'),
      authorizeMessage('10103', '701'),
    ]);
    expect(responses.map(response => response.json().data.accepted).sort()).toEqual([false, true]);
    expect(await prisma.telegramInboundMessage.count({
      where: { telegramChatId: '10103', telegramMessageId: '701' },
    })).toBe(1);
  });

  it('requires message id and Telegram date together when either is supplied', async () => {
    const { code } = await createChallenge();
    expect((await consume(code, '10104')).statusCode).toBe(200);
    const malformed = await app.inject({
      method: 'POST',
      url: '/internal/agent/telegram/message/authorize',
      headers: { authorization: `Bearer ${AGENT_TOKEN}` },
      payload: {
        telegramUserId: '10104',
        telegramChatId: '10104',
        chatType: 'private',
