...
/**
 * How long the push service may keep trying to deliver before dropping the
 * message. FCM (and Expo relaying to it) defaults to 4 weeks; a phone that was
 * offline when a reminder fired would then be handed a days-old alert the
 * moment it reconnects. These are due-moment reminders - one that could not be
 * delivered within a day is stale noise. Matches the worker's STALE_AFTER_MS
 * window in ../reminders/delivery.ts, and the web-push TTL in ./service.ts.
 */
