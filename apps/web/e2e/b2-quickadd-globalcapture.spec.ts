...
test('global quick capture uses responsive in-app date and time pickers', async ({ page }) => {
  mkdirSync(EVIDENCE_DIR, { recursive: true });
  await page.setViewportSize({ width: 1440, height: 900 });
  await register(page);
  ...
  await page.keyboard.press('Control+k');
  const capture = page.getByTestId('global-quick-capture');
  await expect(capture).toBeVisible();
  await expect(capture.locator('input[type="date"]')).toHaveCount(0);
  await expect(capture.locator('input[type="time"]')).toHaveCount(0);

  await page.getByTestId('quick-add-date').click();
  const dateDialog = page.getByRole('dialog', { name: 'Date' });
  await expect(dateDialog).toBeVisible();
  await page.screenshot({ path: `${EVIDENCE_DIR}/command-date-picker-375.png`, fullPage: true });
  const popupBox = await dateDialog.boundingBox();
  expect(popupBox).not.toBeNull();
  expect(popupBox!.x).toBeGreaterThanOrEqual(8);
  expect(popupBox!.x + popupBox!.width).toBeLessThanOrEqual(367);

  await page.keyboard.press('Escape');
  await expect(dateDialog).toBeHidden();
  await expect(capture).toBeVisible();

  await page.getByTestId('quick-add-time').click();
  await page.getByRole('option', { name: '09:30' }).click();
  await expect(page.getByTestId('quick-add-time')).toContainText('09:30');

  await page.evaluate(() => document.documentElement.classList.add('dark'));
  await page.getByTestId('quick-add-date').click();
  await expect(dateDialog).toBeVisible();
  await page.screenshot({ path: `${EVIDENCE_DIR}/command-date-picker-dark-375.png`, fullPage: true });
});
