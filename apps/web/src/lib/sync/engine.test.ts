...
describe('web sync status', () => {
  ...
  it('reports queued changes and clears them after a successful retry', async () => {
    ...
    enqueueMutation('task', 'task-1', 'upsert', { title: 'Updated' }, new Date().toISOString());
    ...
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({
        success: true,
        data: { accepted: ['task-1'], serverTime: '2026-08-02T02:00:00.000Z' },
    ...
    const pushBody = JSON.parse(String(fetchMock.mock.calls[0][1]?.body));
    expect(pushBody.items[0]).toMatchObject({ version: 3, data: { patch: { title: { value: 'Updated' } } } });
  ...
  it('debounces local changes and syncs them after 1.5 seconds', async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      if (String(input).endsWith('/sync/push')) {
        return new Response(JSON.stringify({
          success: true,
          data: { accepted: ['task-2'], serverTime: '2026-08-02T03:00:01.000Z' },
        }), { status: 200, headers: { 'Content-Type': 'application/json' } });
      }
      return new Response(JSON.stringify({
        success: true,
    ...
    enqueueMutation('task', 'task-2', 'upsert', { title: 'Debounced' }, new Date().toISOString());
