...
describe('TelegramConnectCard', () => {
  ...
  it('renders the connection action in Vietnamese', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Cách dùng Mindoist trên Telegram' })).toBeInTheDocument();
    expect(screen.getByText('Hôm nay tôi có những công việc gì?')).toBeInTheDocument();
  });

  it('shows practical examples and the confirmation rule', async () => {
    vi.mocked(getTelegramStatus).mockResolvedValue({ state: 'unlinked', botUsername: 'MindoistTestBot' });
    renderCard();

    expect(await screen.findByTestId('telegram-usage-guide')).toBeInTheDocument();
    ...
    expect(screen.getByText(/nothing is saved until you reply confirm/)).toBeInTheDocument();
  });
});
