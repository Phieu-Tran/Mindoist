      ...
      if (item.table === 'task') {
        if (isDeleted) {
          ...
          await db.tasks.put({
            ...
            dueDate: (item.data.dueDate as string) ?? null,
            ...
            dueTime: (item.data.dueTime as string) ?? null,
            durationMin: (item.data.durationMin as number) ?? null,
...
async function pushMutations(mutations: PendingMutation[]): Promise<PushResult> {
  const items: SyncItem[] = mutations.map(m => ({
    table: m.table,
    id: m.table === 'taskTag' ? m.entityId : m.entityId,
    data: m.data,
...
export async function syncOnce(force = false): Promise<boolean> {
  ...
  try {
    ...
    if (pending.length > 0) {
      ...
      await clearPendingMutations(accepted);
