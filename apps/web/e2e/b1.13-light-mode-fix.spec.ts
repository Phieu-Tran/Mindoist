...
async function assertTodayCellHasBackground(page: Page) {
  const result = await page.evaluate(() => {
    const todayCell = document.querySelector('.mindoist-month-day.is-today');
    if (!todayCell) return { found: false };
    const todayNum = todayCell.querySelector('.mindoist-month-day-number');
...
async function assertHeaderNotWhite(page: Page) {
  const headerBg = await page.evaluate(() => {
    const header = document.querySelector('.mindoist-month-weekday');
