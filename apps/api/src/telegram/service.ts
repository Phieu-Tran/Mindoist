...
export async function authorizeTelegramMessage(telegramUserId: string, telegramChatId: string) {
  const connection = await prisma.telegramConnection.findUnique({
  ...
  if (Date.now() - connection.lastSeenAt.getTime() >= LAST_SEEN_WRITE_INTERVAL_MS) {
    await prisma.telegramConnection.update({
      where: { id: connection.id },
      data: { lastSeenAt: new Date() },
    });
  }
  return { connectionId: connection.id, userId: connection.userId };
}
