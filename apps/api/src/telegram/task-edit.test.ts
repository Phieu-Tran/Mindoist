...
describe('Telegram task edit drafts', () => {
  ...
  beforeEach(async () => {
    ...
    const task = await prisma.task.create({
      data: { userId: userA.user.id, title: 'Kiểm tra DNS', dueDate: new Date('2026-08-04T00:00:00.000Z'), dueTime: '15:00' },
    });
    taskId = task.id;
  });
  ...
  it('confirms the edit and applies only the changed field', async () => {
    ...
    expect(task.dueDate?.toISOString().slice(0, 10)).toBe('2026-08-04');
  });

  it('returns an ambiguous candidate list without creating a draft when multiple tasks match', async () => {
    await prisma.task.create({ data: { userId: userA.user.id, title: 'Kiểm tra DNS lại', dueDate: new Date('2026-08-05T00:00:00.000Z') } });
    ...
    expect(await prisma.telegramTaskEditDraft.count({ where: { userId: userA.user.id } })).toBe(0);
  });
  ...
  it('replaces a pending create-task draft when an edit draft is prepared, and vice versa', async () => {
    ...
    expect(await prisma.telegramTaskDraft.count({ where: { userId: userA.user.id, status: 'PENDING' } })).toBe(1);

    await prepare({ telegramChatId: '2001', locale: 'vi', searchText: 'Kiểm tra DNS', dueTime: '17:00' });
    expect(await prisma.telegramTaskDraft.count({ where: { userId: userA.user.id, status: 'PENDING' } })).toBe(0);
    expect(await prisma.telegramTaskEditDraft.count({ where: { userId: userA.user.id, status: 'PENDING' } })).toBe(1);
  });
