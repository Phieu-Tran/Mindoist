...
describe('Telegram task draft confirmation', () => {
  ...
  beforeAll(async () => {
    ...
    const staleUsers = await prisma.user.findMany({
      where: { email: { in: ['telegram-draft-a@example.com', 'telegram-draft-b@example.com'] } },
      select: { id: true },
    });
    await cleanupUsers(staleUsers.map(({ id }) => id));
  ...
  it('prepares a fixed form without creating a task and resolves only an owned project', async () => {
    ...
    const draft = await prisma.agentDraft.findFirst({ where: { userId: userA.user.id } });
    expect(draft).toMatchObject({ kind: 'CREATE_TASK', status: 'PENDING', transport: 'TELEGRAM' });
    expect(draft?.payload).toMatchObject({ projectId: projectAId, title: 'Gửi báo cáo' });
  ...
  it('replaces the pending draft and confirms exactly once under concurrent retry', async () => {
    ...
    expect(await prisma.agentDraft.count({ where: { userId: userA.user.id, status: 'CANCELLED' } })).toBe(1);
  });
  ...
  it('persists an explicit color and existing owned tags only after confirmation', async () => {
    ...
    expect(task).toMatchObject({ color: 'rose', projectId: projectAId, deadlineTime: '11:00' });
  ...
  it('keeps exactly one pending draft when prepare requests race', async () => {
    ...
    expect(await prisma.agentDraft.count({
  ...
  it('previews and atomically confirms a batch with an explicitly requested new tag', async () => {
    ...
    expect(tasks.every(task => task.projectId === projectAId && task.color === 'rose' && task.deadlineTime === '10:30')).toBe(true);
  ...
  it('keeps one pending write action across single and batch drafts', async () => {
    ...
    expect(await prisma.agentDraft.count({
      where: { userId: userA.user.id, kind: 'CREATE_TASK_BATCH', status: 'CANCELLED' },
    })).toBe(1);

    await prepareBatch({ telegramChatId: '1001', locale: 'vi', titles: ['Batch wins A', 'Batch wins B'] });
    expect(await prisma.agentDraft.count({
      where: { userId: userA.user.id, kind: 'CREATE_TASK', status: 'PENDING' },
    })).toBe(0);
    expect(await prisma.agentDraft.count({
      where: { userId: userA.user.id, kind: 'CREATE_TASK_BATCH', status: 'PENDING' },
    })).toBe(1);
  });
