...
describe('Settings', () => {
  ...
  it('PATCH /settings updates hiddenNavItems', async () => {
    const res = await app.inject({
      method: 'PATCH',
      url: '/settings',
      headers: { authorization: `Bearer ${userToken}` },
      payload: { hiddenNavItems: ['notes', 'countdown'] },
    });
    expect(res.statusCode).toBe(200);
    expect(JSON.parse(res.body).data.hiddenNavItems).toEqual(['notes', 'countdown']);
  });
