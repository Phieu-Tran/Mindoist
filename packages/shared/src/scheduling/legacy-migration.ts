...
export function planLegacyScheduleMigration(
  ...
  const duration = task.estimateMin ?? task.durationMin;

  if (duration != null && (!Number.isInteger(duration) || duration <= 0)) {
    warnings.push('invalid-duration');
  } else if (duration && dueDate && validTime) {
    // Legacy dueTime represents the completion moment. Preserve that
    // meaning when creating a planned block by ending the block at the
    // deadline and moving its start backwards by the estimate.
    const endLocal = `${dueDate}T${dueTime}`;
    timeBlockCandidate = {
      source: 'legacy-timed-duration',
      startLocal: addMinutes(endLocal, -duration),
      endLocal,
      timeZone,
      allDay: false,
    };
