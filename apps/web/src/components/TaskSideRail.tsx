...
import { PersistentResizablePanel } from './PersistentResizablePanel';
...
export function TaskSideRail({
  ...
  const [selectedKey, setSelectedKey] = useState(() => selectedTask?.deadline?.date || todayKey);
  const [railExpanded, setRailExpanded] = useState(false);
  const projectMap = useMemo(() => new Map(projects.map(project => [project.id, project])), [projects]);
  const identityColorFor = (task: Task) => effectiveTaskColor(
    task.color,
  ...
  const tasksByDay = useMemo(() => {
    ...
    tasks.forEach(task => {
      if (!task.deadline || task.parentId) return;
      const key = task.deadline.date;
  ...
  const selectedTaskDate = selectedTask?.deadline?.date ?? null;
  ...
  useEffect(() => {
    ...
    const key = selectedTask?.deadline?.date ?? todayKey;
    const date = new Date(`${key}T12:00:00`);
    setSelectedKey(key);
    setVisibleMonth(new Date(date.getFullYear(), date.getMonth(), 1, 12));
  }, [selectedTask?.id, selectedTask?.deadline?.date, todayKey]);
              ...
              {dayTasks.length > 0 && (
                <span className="mt-0.5 flex max-w-7 gap-0.5" aria-hidden="true">
                  {dayTasks.slice(0, 3).map(task => (
                    <span key={task.id} className={cn('task-color-dot h-1 w-1 rounded-full', taskColorClass(identityColorFor(task)))} />
                  ))}
                </span>
              )}
            </button>
        ...
        {selectedTasks.length === 0 ? (
          ...
          <div className="flex flex-col gap-1.5">
            {selectedTasks.slice(0, 4).map(task => (
              ...
              >
                <span className={cn('task-color-dot h-2 w-2 rounded-full', taskColorClass(identityColorFor(task)))} aria-hidden="true" />
                <span className="min-w-0 flex-1 truncate">{task.title}</span>
                {task.deadline?.time && <span className="text-xs text-muted-foreground">{task.deadline.time}</span>}
              </button>
            ))}
          </div>
        )}
  ...
  return (
    <PersistentResizablePanel
      testId="task-side-rail"
      label={t('summary.railLabel', 'Task details')}
      storageKey="mindoist:panel:task-rail-width-v2"
      defaultWidth={448}
      minWidth={384}
      maxWidth={680}
      resizeEdge="left"
      className="hidden xl:flex sticky top-0 z-20 h-[calc(100vh-2rem)] shrink-0 flex-col overflow-hidden pl-4"
    >
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-panel border border-border/60 bg-card">
