...
describe('SummaryDashboard', () => {
  ...
  it('shows a Summary dashboard with period controls, trend, priority, project, and calendar panels', () => {
    ...
    expect(screen.getByTestId('summary-month-total')).toHaveTextContent('4');
  ...
  it('filters period metrics by project and can switch to year view', () => {
    ...
    const monthlyList = within(screen.getByTestId('summary-monthly-list'));
    expect(monthlyList.getByText('Task 1')).toBeInTheDocument();
    expect(monthlyList.queryByText('Task 3')).not.toBeInTheDocument();
  });
